                ...
                @foreach ($games as $game)
                    <tbody class="text-gray-700 text-sm font-medium dark:text-gray-300">
                        <tr wire:key="{{ $game->gameId }}" wire:click="toggle({{ $game->gameId }})"
                            class="border-b border-gray-300 dark:border-gray-600 dark:hover:bg-indigo-600 duration-200 cursor-pointer {{ $expandedGameId == $game->gameId ? 'h-44 bg-indigo-600' : 'h-4' }}">
                            <td
